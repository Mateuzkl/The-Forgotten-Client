...
This branch targets direct TFC mode only: ElfBot attaches to the real TFC/Tibia process and reads the mirrored Tibia 8.60 memory from that process.

## Legacy stub path

Older experimental code for `TibiaStub.exe` and `Local\TibiaShim860` still exists in the repository, but it is out of scope for this branch.

For this branch, TFC does not launch the stub and does not create the IPC mapping. Tests should attach ElfBot to the real `Tibia.exe` generated by TFC.
...
- The active path in this branch is direct TFC mode.
