...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
		...
		if (casterPlayer) {
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				...
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
					damage.primary.value += std::round(damage.primary.value * (skill / 250.));
					damage.secondary.value += std::round(damage.secondary.value * (skill / 250.));
	...
	if (success) {
		...
		if (damage.critical) {
			...
			// Animated text on critical hit
			std::ostringstream critText;
			critText << "CRIT: " << std::abs(damage.primary.value + damage.secondary.value);
			g_game.addAnimatedText(critText.str(), target->getPosition(), TEXTCOLOR_YELLOW);
		}
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	if (!damage.critical && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
		...
		if (chance > 0 && skill > 0 && uniform_random(1, 100) <= chance) {
			double aoeCritMultiplier = 0.40; // 80% of the normal crit bonus (20% nerf)
			criticalPrimary = std::round(damage.primary.value * (skill / 200.) * aoeCritMultiplier);
			criticalSecondary = std::round(damage.secondary.value * (skill / 200.) * aoeCritMultiplier);
			damage.critical = true;
		}
	}
