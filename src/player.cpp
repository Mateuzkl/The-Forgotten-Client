...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (accountManager != AccountManagerMode::None) {
	...
	if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
		...
		if (idleTime > (kickAfterMinutes * 60000) + 60000) {
			...
			client->sendTextMessage(TextMessage(
				...
				fmt::format("There was no variation in your behaviour for {:d} minutes. "
					"You will be disconnected in one minute if there is no change in your actions until then.",
					kickAfterMinutes)
			));
		}
	}

	// Precise frag/skull countdown (seconds), per-player (uses member skullFragAcc)
	skullFragAcc += static_cast<double>(interval) / 1000.0; // ms -> s
	if (skullFragAcc >= 1.0) {
		if (g_game.getWorldType() != WORLD_TYPE_PVP_ENFORCED) {
			const int64_t wholeSeconds = static_cast<int64_t>(skullFragAcc);
			checkSkullTicks(wholeSeconds);   // skullTicks are in SECONDS
			skullFragAcc -= wholeSeconds;    // keep fractional remainder to avoid “stuck at X sec”
		}
		else {
			skullFragAcc = 0.0;
		}
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (sendText) {
		...
		g_game.addAnimatedText(std::to_string(exp), position, TEXTCOLOR_CYAN);
...
uint64_t Player::getGainedExperience(Creature* attacker) const
{
	if (g_config.getBoolean(ConfigManager::EXPERIENCE_FROM_PLAYERS)) {
		...
		if (attackerPlayer && attackerPlayer != this && skillLoss) {
			...
			// ✅ Disable exp if not allowed by config
			if (!g_config.getBoolean(ConfigManager::GAIN_EXP_ON_PARTY_MEMBERS) &&
				attackerPlayer->getParty() && attackerPlayer->getParty() == getParty()) {
				return 0;
			}

			if (!g_config.getBoolean(ConfigManager::GAIN_EXP_ON_GUILD_MEMBERS) &&
				attackerPlayer->getGuild() && attackerPlayer->getGuild() == getGuild()) {
				return 0;
			}

			if (!g_config.getBoolean(ConfigManager::GAIN_EXP_ON_SAME_IP_MEMBERS) &&
				attackerPlayer->getIP() == getIP()) {
				return 0;
			}

			// ✅ Level range check
			if (std::abs(static_cast<int32_t>(attackerPlayer->getLevel() - level)) <=
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	// Store frag time in SECONDS (simple & consistent)
	const int64_t fragTimeSeconds = std::max<int64_t>(1, g_config.getNumber(ConfigManager::FRAG_TIME));
	skullTicks += fragTimeSeconds; // +1 frag worth of time

	const int64_t killsToRed = g_config.getNumber(ConfigManager::KILLS_TO_RED);
	const int64_t killsToBlack = g_config.getNumber(ConfigManager::KILLS_TO_BLACK);

	// Match classic TFS logic: trigger when skullTicks > (kills-1)*fragTime
	// (so red activates exactly on the Nth kill)
	if (getSkull() != SKULL_BLACK) {
		if (killsToBlack > 0 && skullTicks > (killsToBlack - 1) * fragTimeSeconds) {
			setSkull(SKULL_BLACK);
		}
		else if (getSkull() != SKULL_RED && killsToRed > 0 && skullTicks > (killsToRed - 1) * fragTimeSeconds) {
			setSkull(SKULL_RED);
		}
	}
}
...
{
	skullTicks = std::max<int64_t>(0, skullTicks - ticks); // skullTicks are SECONDS

	// Auto-clear skull the moment all frags expire
	if (skullTicks == 0 && (skull == SKULL_RED || skull == SKULL_BLACK)) {
		setSkull(SKULL_NONE);
		sendTextMessage(MESSAGE_STATUS_WARNING, "All unjustified kills have expired. Your skull has been cleared.");
	}
}
...
{
	if ((OTSYS_TIME() - lastToggleMount) < g_config.getNumber(ConfigManager::TOGGLE_MOUNT_DELAY) && !wasMounted) {
...
void Player::handleNamelockManager(const std::string& text, std::ostringstream& msg, bool& shouldShowHelp)
{
	...
	auto isAllowedChar = [](char c) -> bool {
		// PHP allowed: letters + "- '"
		return std::isalpha(static_cast<unsigned char>(c)) || c == '-' || c == ' ' || c == '\'';
		};

	auto ucwordsLikePhp = [](const std::string& lower) -> std::string {
		// PHP: ucwords() uses whitespace as separators by default.
	...
	auto validateAjaxNameRules = [&](const std::string& rawInput, std::string& formattedName, std::string& error) -> bool {
		// PHP: $name = strtolower(trim($_REQUEST['name']));
		std::string name = rawInput;
		trimString(name);
		name = toLowerCopy(name);

		if (name.empty()) {
			error = "Account Manager: Please enter new character name.";
			return false;
		}

		// PHP strspn allowed set
		for (char c : name) {
			if (!isAllowedChar(c)) {
				error = "Account Manager: Name contains illegal letters. Use only: letters, spaces, -, '.";
				return false;
			}
		}
		...
		if (name.length() > 25) {
			error = "Account Manager: Too long name. Max. lenght 25 letters.";
			return false;
		}

		// PHP: $names_blocked = array('gm','cm','god','tutor','admin','adm');
		static const std::vector<std::string> names_blocked = {
			"gm", "cm", "god", "tutor", "admin", "adm"
		};
		for (const auto& w : names_blocked) {
			if (name == w) {
				error = "Account Manager: Blocked name.";
				return false;
			}
		}

		// PHP: $first_words_blocked = array('gm ','cm ', 'god ','tutor ', 'admin', 'adm', \"'\", '-');
		static const std::vector<std::string> first_words_blocked = {
			"gm ", "cm ", "god ", "tutor ", "admin", "adm", "'", "-"
		};
		for (const auto& p : first_words_blocked) {
			if (name.size() >= p.size() && name.compare(0, p.size(), p) == 0) {
				error = "Account Manager: First letters in name can't be blocked words/symbols.";
				return false;
			}
		}

		// PHP: if(substr($name, -1) == \"'\" || substr($name, -1) == \"-\")
		if (!name.empty()) {
			char last = name.back();
			if (last == '\'' || last == '-') {
				error = "Account Manager: Last letter can't be ' and -.";
				return false;
			}
		}

		// PHP: $words_blocked (substring match anywhere)
		static const std::vector<std::string> words_blocked = {
			"gamemaster", "game master", "game-master", "game'master",
			"  ", "--", "''", "' ", " '", "- ", " -", "-'", "'-",
			"fuck", "sux", "suck", "noob", "tutor", "admin", "adm", "nigga"
		};
		for (const auto& w : words_blocked) {
			if (name.find(w) != std::string::npos) {
				error = "Account Manager: Name can't cointain blocked words/patterns.";
				return false;
			}
		}

		// PHP: can't contain 3 same letters one by one
		for (size_t i = 0; i + 2 < name.size(); ++i) {
			if (name[i] == name[i + 1] && name[i] == name[i + 2]) {
				error = "Account Manager: Name can't contain 3 same letters one by one.";
				return false;
			}
		}

		// PHP: if($name[$i-1] == ' ' && $name[$i+1] == ' ')
		// emulate safely: i in [1..len-2]
		for (size_t i = 1; i + 1 < name.size(); ++i) {
			if (name[i - 1] == ' ' && name[i + 1] == ' ') {
				error = "Account Manager: Use normal name format.";
				return false;
			}
		}

		// PHP: if(substr($name, 1, 1) == ' ')
		if (name.size() >= 2 && name[1] == ' ') {
			error = "Account Manager: Use normal name format.";
			return false;
		}

		// PHP: if(substr($name, -2, 1) == ' ')
		if (name.size() >= 2 && name[name.size() - 2] == ' ') {
			error = "Account Manager: Use normal name format.";
			return false;
		}

		// PHP output shows ucwords($name)
		formattedName = ucwordsLikePhp(name);
		return true;
		};
	...
	if (!managerTalkState[1]) {
		...
		if (!validateAjaxNameRules(text, formatted, error)) {
			msg << error;
			sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, msg.str());
			return;
		}

		// Keep same behavior as PHP: check DB existence after validation
		if (IOLoginData::playerNameExists(formatted)) {
			msg << "Account Manager: Player with this name already exist.";
			sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, msg.str());
			return;
		}

		managerData.string1 = formatted;
		managerTalkState[1] = true;
		managerTalkState[2] = true;
		msg << "Account Manager: " << formatted << ", are you sure?";
		sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, msg.str());
		return;
	}
		...
		if (!IOLoginData::playerNameExists(managerData.string1)) {
			...
			if (Database::getInstance().executeQuery(fmt::format(
				...
				Database::getInstance().executeQuery(fmt::format(
					"DELETE FROM `player_namelocks` WHERE `player_id` = {:d}", guid));
...
void Player::handleAccountManager(const std::string& text, std::ostringstream& msg, bool& shouldShowHelp)
{
	...
	if (checkText(text, "cancel") || (checkText(text, "account") && !managerTalkState[1])) {
		...
		int32_t vocationId = g_vocations.getVocationId(text);
		if (vocationId != -1) {
...
void Player::handleNewAccountManager(const std::string& text, std::ostringstream& msg, bool& shouldShowHelp)
{
	...
	if (managerTalkState[7]) {
		...
		std::string storedKeyHash = result->getString("secret");
		...
		std::string inputKeyHash = transformToSHA1(recoveryKey);
	...
	if (checkText(text, "yes") && managerTalkState[3]) {
		if (g_config.getBoolean(ConfigManager::GENERATE_ACCOUNT_NUMBER)) {
			...
			if (accountCreated) {
				accountManager = AccountManagerMode::Account;
...
void Player::manageAccount(const std::string& text)
{
	...
	case AccountManagerMode::Namelock:
		handleNamelockManager(text, msg, shouldShowHelp);
		break;

	case AccountManagerMode::Account:
		handleAccountManager(text, msg, shouldShowHelp);
		break;

	case AccountManagerMode::New:
