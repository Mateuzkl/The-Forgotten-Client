...
void Monster::setLevel(int32_t newLevel)
{
	if (newLevel <= 0)
		level = 1;
	else
	...
	// update description
	nameDescription = mType->nameDescription + " [Level " + std::to_string(level) + "]";
}


void Monster::applyLevelBonuses()
{
	// reset base stats
	healthMax = mType->info.healthMax;
	health = mType->info.health;
	...
	// apply level scaling from config
	float hpBonus = g_config.getFloat(ConfigManager::MLVL_BONUSHP) * level;
	float speedBonus = g_config.getFloat(ConfigManager::MLVL_BONUSSPEED) * level;

	if (hpBonus != 0.0f) {
		healthMax += healthMax * hpBonus;
		health += health * hpBonus;
	}

	if (speedBonus != 0.0f) {
		baseSpeed += baseSpeed * speedBonus;
	}
}
		...
		for (const auto& [playerId, scoreInfo] : rewardBossContributionInfo[monsterId].playerScoreTable) {
			...
			if (hasLoot) {
				if (player) {
					...
					player->sendTextMessage(MESSAGE_INFO_DESCR, "The following items dropped by " + getMonster()->getName() + " are available in your reward chest: " + rewardContainer->getContentDescription() + ".");
				}
				else {
				...
				player->sendTextMessage(MESSAGE_INFO_DESCR, "You did not receive any loot.");
			}
		}
