...
void LuaScriptInterface::registerFunctions()
{
	...
	registerGlobalVariable("ACCOUNT_MANAGER_NONE", static_cast<uint8_t>(AccountManagerMode::None));
	registerGlobalVariable("ACCOUNT_MANAGER_NEW", static_cast<uint8_t>(AccountManagerMode::New));
	registerGlobalVariable("ACCOUNT_MANAGER_ACCOUNT", static_cast<uint8_t>(AccountManagerMode::Account));
	registerGlobalVariable("ACCOUNT_MANAGER_NAMELOCK", static_cast<uint8_t>(AccountManagerMode::Namelock));
	...
	registerEnum(CONST_ANI_SUPERROYALSTAR)
	registerEnum(CONST_ANI_ROYALSTAR)
	registerEnum(CONST_ANI_SUPREMESTAR)
	registerEnum(CONST_ANI_SUPREMESTAR)
	registerEnum(CONST_ANI_SUPREMEROYALSTAR)
	registerEnum(CONST_ANI_ENCHANTEDDONATESTAR)
	...
	registerMethod("Player", "getAccountManagerMode", LuaScriptInterface::luaPlayerGetAccountManagerMode);
	...
	registerMethod("Player", "addAura", LuaScriptInterface::luaPlayerAddAura);
	registerMethod("Player", "getCurrentAura", LuaScriptInterface::luaPlayerGetCurrentAura);
	registerMethod("Player", "setCurrentAura", LuaScriptInterface::luaPlayerSetCurrentAura);
...
{
	// player:sendAnimatedText(message, position, color)
	...
	Position pos = getPosition(L, 3);
	int color = getNumber<int>(L, 4);
	player->sendAnimatedText(message, pos, static_cast<TextColor_t>(color));
	pushBoolean(L, true);
	return 1;
}
...
{
	// item:serializeAttributes() → string (binary blob)
	Item* item = getUserdata<Item>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
	...
	PropWriteStream prop;
	item->serializeAttr(prop);

	size_t size;
	const char* data = prop.getStream(size);
...
{
	// item:unserializeAttributes(blob)
	Item* item = getUserdata<Item>(L, 1);
	if (!item) {
	...
	PropStream prop;
	prop.init(buf, len);
	bool ok = item->unserializeAttr(prop);
...
int LuaScriptInterface::luaPlayerGetAccountManagerMode(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerHasAura(lua_State* L) {
	...
	const Aura* aura = g_game.auras.getAuraByID(auraId);
	if (!aura) {
		pushBoolean(L, false);
		return 1;
	}
...
int LuaScriptInterface::luaPlayerGetCurrentAura(lua_State* L) {
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnumber(L, 0);
		return 1;
	}
	lua_pushnumber(L, player->getCurrentAura());
	return 1;
}

int LuaScriptInterface::luaPlayerSetCurrentAura(lua_State* L) {
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		pushBoolean(L, false);
		return 1;
...
int LuaScriptInterface::luaMonsterSetLevel(lua_State* L)
{
	...
	int32_t newLevel = getNumber<int32_t>(L, 2, -1);
	if (newLevel <= 0) {
		lua_pushboolean(L, false);
		return 1;
	}
