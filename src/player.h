...
static constexpr int32_t PLAYER_MAX_SPEED = 1700;
static constexpr int32_t PLAYER_GM_MAX_SPEED = 2500; // Max speed for GMs/GMs
static constexpr int32_t PLAYER_MIN_SPEED = 1;
enum class AccountManagerMode : uint8_t {
	None = 0,
	New,
	Account,
	Namelock
...
class Player final : public Creature, public Cylinder
{
	public:
		...
		bool isAccountManager() const { return accountManager != AccountManagerMode::None; }
	...
	private:
		...
		int32_t idleTime = 0;
		double skullFragAcc = 0.0; // accumulates think time in seconds for frag countdown
		...
		AccountManagerMode accountManager{ AccountManagerMode::None };
		...
		int32_t getStepSpeed() const override {
			int32_t maxSpeed = (group && group->id >= 4) ? PLAYER_GM_MAX_SPEED : PLAYER_MAX_SPEED;
			return std::max<int32_t>(PLAYER_MIN_SPEED, std::min<int32_t>(maxSpeed, getSpeed()));
		}
		void updateBaseSpeed() {
			if (group && group->id >= 3) { // GM (3) or GOD (4) or any higher
				baseSpeed = PLAYER_GM_MAX_SPEED; // both will always have 2500
			} else {
